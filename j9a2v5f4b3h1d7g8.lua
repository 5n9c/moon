...
local Button_Icon = "rbxassetid://106832030427465"
...
local Components = {
	Assets = {
		...
		Min = "rbxassetid://110485023487026",
...
Components.Tab = (function()
	...
	function TabModule:GetCurrentTabPos()
		if not TabModule.Tabs[TabModule.SelectedTab] then return 0 end
		local TabHolderPos = TabModule.Window.TabHolder.AbsolutePosition.Y
		local TabPos = TabModule.Tabs[TabModule.SelectedTab].Frame.AbsolutePosition.Y
		local TabSize = TabModule.Tabs[TabModule.SelectedTab].Frame.AbsoluteSize.Y
		return (TabPos - TabHolderPos) + (TabSize / 2)
	end

	function TabModule:New(Title, Icon, Parent)
		local Window, Elements = TabModule.Window, Library.Elements
		...
		if not fischbypass then
			if Library:GetIcon(Icon) then iconId = Library:GetIcon(Icon)
			else
				warn(("Ícone '%s' não encontrado para a aba '%s'."):format(Icon, Title))
				iconId = "rbxassetid://10734965702"
			end
		end

		Tab.Frame = New("TextButton", {
			Size = UDim2.fromOffset(42, 42), BackgroundTransparency = 1, Parent = Parent, Text = "",
			ThemeTag = { BackgroundColor3 = "Tab" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 8) }),
			New("ImageLabel", {
				AnchorPoint = Vector2.new(0.5, 0.5), Size = UDim2.fromOffset(22, 22), Position = UDim2.fromScale(0.5, 0.5),
		...
		Tab.ContainerFrame = New("ScrollingFrame", {
			...
			ScrollBarImageColor3 = Color3.fromRGB(255, 255, 255), ScrollBarImageTransparency = 0.95, ScrollBarThickness = 3,
			BorderSizePixel = 0, CanvasSize = UDim2.fromScale(0, 0), ScrollingDirection = Enum.ScrollingDirection.Y,
		}, {
			ContainerLayout, New("UIPadding", { PaddingRight = UDim.new(0, 10), PaddingLeft = UDim.new(0, 6.5), PaddingTop = UDim.new(0, -1), PaddingBottom = UDim.new(0, 1) }),
		})
		...
		local textIcon = Tab.Frame:FindFirstChildOfClass("ImageLabel")
		local colorMotor = Flipper.SingleMotor.new(0)
		
		colorMotor:onStep(function(v)
			if textIcon then
				local subTextColor = Creator.GetThemeProperty("SubText")
				local darkerSubText = Color3.new(subTextColor.R * 0.7, subTextColor.G * 0.7, subTextColor.B * 0.7)
				
				textIcon.ImageColor3 = darkerSubText:Lerp(Creator.GetThemeProperty("Text"), v)
			end
		end)
		
		function Tab.UpdateSelectionStyle(selected)
			Tab.Selected = selected
			Tab.SetTransparency(selected and 0.89 or 1)
			colorMotor:setGoal(Flipper.Spring.new(selected and 1 or 0, { frequency = 5 }))
		end

		Creator.AddSignal(Tab.Frame.MouseEnter, function() if not Tab.Selected then Tab.SetTransparency(0.92); colorMotor:setGoal(Flipper.Spring.new(0.5, { frequency = 8 })) end end)
		Creator.AddSignal(Tab.Frame.MouseLeave, function() if not Tab.Selected then Tab.SetTransparency(1); colorMotor:setGoal(Flipper.Spring.new(0, { frequency = 8 })) end end)
		Creator.AddSignal(Tab.Frame.MouseButton1Down, function() Tab.SetTransparency(0.85) end)
		Creator.AddSignal(Tab.Frame.MouseButton1Up, function() Tab.SetTransparency(Tab.Selected and 0.89 or 0.92) end)
		Creator.AddSignal(Tab.Frame.MouseButton1Click, function() TabModule:SelectTab(TabIndex) end)

		TabModule.Containers[TabIndex], TabModule.Tabs[TabIndex] = Tab.ContainerFrame, Tab
		Tab.Container, Tab.ScrollFrame = Tab.ContainerFrame, Tab.Container

		function Tab:AddSection(SectionTitle, SectionIcon)
			local Section = { Type = "Section" }
			local icon = SectionIcon
			if not fischbypass and Library:GetIcon(icon) then icon = Library:GetIcon(icon) elseif icon == "" or icon == nil then icon = nil end
			local SectionFrame = Components.Section(SectionTitle, Tab.Container, icon)
			Section.Container, Section.ScrollFrame = SectionFrame.Container, Tab.Container
	...
	function TabModule:SelectTab(Tab)
		...
		for i, TabObject in pairs(TabModule.Tabs) do TabObject.UpdateSelectionStyle(i == Tab) end

		Window.TabDisplay.Text = TabModule.Tabs[Tab].Name
		Window.SelectorPosMotor:setGoal(Spring(TabModule:GetCurrentTabPos(), { frequency = 8, dampingRatio = 0.8 }))
		...
		task.spawn(function()
			...
			task.wait(0.12)
			for i, Container in pairs(TabModule.Containers) do Container.Visible = (i == Tab) end
	...
	return function(Config)
		...
		local function BarButton(Icon, Pos, Parent, Callback)
			...
			Button.Frame = New("TextButton", {
				...
				BackgroundTransparency = 1, Parent = Parent, Position = Pos, Text = "", ZIndex = 3,
				ThemeTag = { BackgroundColor3 = "Text" },
			}, {
				New("UICorner", { CornerRadius = UDim.new(0, 7) }),
				New("ImageLabel", {
					Image = Icon, Size = UDim2.fromOffset(16, 16), Position = UDim2.fromScale(0.5, 0.5),
					AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, Name = "Icon",
					ThemeTag = { ImageColor3 = "Text" },
				}),
			})
		...
		local IconImageLabel = Config.Icon and New("ImageLabel", {
			Image = Config.Icon,
			Size = UDim2.fromOffset(20, 20),
			Position = UDim2.new(0, 12 + 10, 0.5, 0),
			AnchorPoint = Vector2.new(0.5, 0.5),
			BackgroundTransparency = 1,
			ThemeTag = { ImageColor3 = "Text" },
		}) or nil
		...
		TitleBar.Frame = New("Frame", {
			Size = UDim2.new(1, 0, 0, 42), BackgroundTransparency = 1, Parent = Config.Parent, ZIndex = 2,
		}, {
			IconImageLabel,

			New("Frame", {
				Size = UDim2.new(0.6, 0, 1, 0), Position = UDim2.new(0.2, 0, 0, 0),
				BackgroundTransparency = 1,
			}, {
				New("UIListLayout", {
				...
				New("TextLabel", {
					...
					TextSize = 14, TextXAlignment = "Center", TextYAlignment = "Center",
				...
				Config.SubTitle and New("TextLabel", {
					...
					TextSize = 11, TextXAlignment = "Center", TextYAlignment = "Center",
					Size = UDim2.new(1, 0, 0, 12), BackgroundTransparency = 1, LayoutOrder = 2,
					ThemeTag = { TextColor3 = "Text" },
				}) or nil,
			}),
        ...
        if IconImageLabel then
            ...
            local rotationTween = TweenService:Create(IconImageLabel, tweenInfo, {Rotation = 360})
            rotationTween:Play()
        end
		TitleBar.MinButton = BarButton(Components.Assets.Min, UDim2.new(1, -4, 0, 4), TitleBar.Frame, function()
...
Components.Window = (function()
	...
	return function(Config)
		local Window = {
			...
			Size = Config.Size,
			CurrentPos = 0,
			TabWidth = Config.TabWidth or 80,
		...
		local Dragging, DragInput, MousePos, StartPos = false, nil, Vector2.new(), UDim2.new()
		local Resizing, ResizePos = false, Vector2.new()
		local MinimizeNotif = false
		local OldSizeX, OldSizeY = Config.Size.X.Offset, Config.Size.Y.Offset
		...
		local TabFrame = New("Frame", {
			Size = UDim2.new(0, Window.TabWidth, 1, -65), 
		...
		Window.TabDisplay = New("TextLabel", {
			...
			TextXAlignment = Enum.TextXAlignment.Center,
			TextYAlignment = Enum.TextYAlignment.Center,
			Size = UDim2.new(1, -(Window.TabWidth + 48), 0, 32),
			Position = UDim2.fromOffset(Window.TabWidth + 0, 55),
			BackgroundTransparency = 1,
			ThemeTag = {
				TextColor3 = "Text",
			},
		...
		Window.ContainerCanvas = New("Frame", {
			Size = UDim2.new(1, -(Window.TabWidth + 30), 1, -70),
			Position = UDim2.fromOffset(Window.TabWidth + 20, 54),
			BackgroundTransparency = 0.95,
		...
		Window.SelectorPosMotor:onStep(function(Value)
			Selector.Position = UDim2.new(0, 6, 0, Value)
		end)
		...
		function Window:Maximize(Value, NoPos, IsInstant)
			Window.Maximized = Value
			Window.TitleBar.MaxButton.Frame.Icon.Image = Value and Components.Assets.Restore or Components.Assets.Max
			if not Value then
				OldSizeX, OldSizeY = Window.Root.AbsoluteSize.X, Window.Root.AbsoluteSize.Y
			end
			local SizeX, SizeY = (Value and Camera.ViewportSize.X or OldSizeX), (Value and Camera.ViewportSize.Y or OldSizeY)
			
			SizeMotor:setGoal({
				X = Flipper[IsInstant and "Instant" or "Spring"].new(SizeX, { frequency = 6 }),
				Y = Flipper[IsInstant and "Instant" or "Spring"].new(SizeY, { frequency = 6 }),
			})
			Window.Size = UDim2.fromOffset(SizeX, SizeY)

			if not NoPos then
				local goalX = Value and 0 or Window.Position.X.Offset
				local goalY = Value and 0 or Window.Position.Y.Offset
				PosMotor:setGoal({
					X = Spring(goalX, { frequency = 6 }),
					Y = Spring(goalY, { frequency = 6 }),
				})
			end
		end
		...
		Creator.AddSignal(Window.TitleBar.Frame.InputBegan, function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				if Window.Maximized then
					local mousePercentage = (Input.Position.X - Window.Root.AbsolutePosition.X) / Window.Root.AbsoluteSize.X
					local newX = Input.Position.X - (OldSizeX * mousePercentage)
					StartPos = UDim2.fromOffset(newX, Input.Position.Y - 20)
				end
			end
		end)
		...
		Creator.AddSignal(UserInputService.InputChanged, function(Input)
			if Dragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				...
				PosMotor:setGoal({ X = Instant(newPosition.X.Offset), Y = Instant(newPosition.Y.Offset) })
			...
			if Resizing and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				...
				local newSize = Vector2.new(StartPos.X.Offset + Delta.X, StartPos.Y.Offset + Delta.Y)
				local clampedSize = Vector2.new(math.clamp(newSize.X, 500, 2048), math.clamp(newSize.Y, 400, 2048))
				
				SizeMotor:setGoal({ X = Instant(clampedSize.X), Y = Instant(clampedSize.Y) })
				Window.Size = UDim2.fromOffset(clampedSize.X, clampedSize.Y)
			end
		end)
		...
		Creator.AddSignal(Window.TabHolder.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			Window.TabHolder.CanvasSize = UDim2.new(0, 0, 0, Window.TabHolder.UIListLayout.AbsoluteContentSize.Y + 24)
		end)
		...
		Creator.AddSignal(UserInputService.InputBegan, function(Input)
			...
			if type(Library.MinimizeKeybind) == "table" and Library.MinimizeKeybind.Type == "Keybind" then
				if Input.KeyCode.Name == Library.MinimizeKeybind.Value then keyMatch = true end
		...
		function Window:Minimize()
			...
			if not MinimizeNotif then
				...
				if not Mobile then
					...
					Library:Notify({ Title = "Interface", Content = "Tap the button to toggle the interface.", Duration = 6 })
				end
			end
		end
		...
		function Window:Dialog(Config)
			...
			local Content = New("TextLabel", {
				FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"), Text = Config.Content, TextColor3 = Color3.fromRGB(240, 240, 240), TextSize = 14,
				TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, Size = UDim2.new(1, -40, 1, 0), Position = UDim2.fromOffset(20, 60),
				BackgroundTransparency = 1, Parent = Dialog.Root, ClipsDescendants = false, ThemeTag = { TextColor3 = "Text" },
			})
			...
			if Content.TextBounds.X + 40 > Window.Size.X.Offset - 120 then
				Dialog.Root.Size, Content.TextWrapped = UDim2.fromOffset(Window.Size.X.Offset - 120, 165), true
				Dialog.Root.Size = UDim2.fromOffset(Window.Size.X.Offset - 120, Content.TextBounds.Y + 150)
			end
			for _, Button in ipairs(Config.Buttons) do Dialog:Button(Button.Title, Button.Callback) end
			Dialog:Open()
		end
...
ElementsTable.Button = (function()
	...
	function Element:New(Config)
		...
		local iconToShow = nil

		if Config.Icon then
			iconToShow = Library:GetIcon(Config.Icon) or Config.Icon
		end
...
ElementsTable.Toggle = (function()
	...
	function Element:New(Idx, Config)
		...
		ToggleFrame.DescLabel.Size = UDim2.new(1, -90, 0, 14)
		...
		if not Mobile and Config.Keybind then
			local Keybind = {
				Value = Config.Keybind.Default or "None",
				Mode = Config.Keybind.Mode or "Toggle",
				Callback = function() Toggle:SetValue(not Toggle.Value) end,
				Picking = false,
			}
			...
			local KeybindDisplayButton = New("TextButton", {
				...
				New("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Transparency = 0.5, ThemeTag = { Color = "InElementBorder" } }),
				KeybindDisplayLabel,
			})
			
			Creator.AddSignal(KeybindDisplayButton.MouseButton1Click, function()
				...
				connection = UserInputService.InputBegan:Connect(function(input)
					if Keybind.Picking then
						...
						elseif input.UserInputType == Enum.UserInputType.MouseButton1 then keyName = "MouseLeft"
						elseif input.UserInputType == Enum.UserInputType.MouseButton2 then keyName = "MouseRight" end

						if keyName then
							Keybind.Value = keyName
							KeybindDisplayLabel.Text = keyName
			...
			Creator.AddSignal(UserInputService.InputBegan, function(input)
				if UserInputService:GetFocusedTextBox() or Keybind.Picking or Keybind.Value == "None" then return end
				
				local isKey = (input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode.Name == Keybind.Value)
				local isMouse = (input.UserInputType == Enum.UserInputType.MouseButton1 and Keybind.Value == "MouseLeft") or (input.UserInputType == Enum.UserInputType.MouseButton2 and Keybind.Value == "MouseRight")
		...
		local FillFrame = New("Frame", {
			Size = UDim2.fromScale(0, 0), Position = UDim2.fromScale(0.5, 0.5), AnchorPoint = Vector2.new(0.5, 0.5),
			BorderSizePixel = 0, ThemeTag = { BackgroundColor3 = "Accent" },
		}, { New("UICorner", { CornerRadius = UDim.new(0, 4) }) })

		local Checkmark = New("ImageLabel", {
			Image = "rbxassetid://10709790644", Size = UDim2.fromScale(0.8, 0.8), Position = UDim2.fromScale(0.5, 0.5),
			AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, ImageTransparency = 1, ThemeTag = { ImageColor3 = "ToggleToggled" },
		})

		local ToggleSquare = New("Frame", {
			Size = UDim2.fromOffset(18, 18), AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -10, 0.5, 0),
			Parent = ToggleFrame.Frame, BackgroundTransparency = 1, ClipsDescendants = true,
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 4) }),
			New("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Transparency = 0.5, ThemeTag = { Color = "InElementBorder" } }),
			FillFrame, Checkmark,
		...
		function Toggle:SetValue(Value)
			...
			local targetSize = (Value and UDim2.fromScale(1, 1) or UDim2.fromScale(0, 0))
			local targetTransparency = (Value and 0 or 1)
			local targetBorderColor = (Value and Creator.GetThemeProperty("Accent") or Creator.GetThemeProperty("InElementBorder"))
			local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)

			TweenService:Create(FillFrame, tweenInfo, { Size = targetSize }):Play()
			TweenService:Create(Checkmark, tweenInfo, { ImageTransparency = targetTransparency }):Play()
			TweenService:Create(ToggleSquare.UIStroke, tweenInfo, { Color = targetBorderColor }):Play()
		...
		local initialState = Toggle.Value
		Toggle.Value = not initialState
		Toggle:SetValue(initialState)
		
		FillFrame.Size = (initialState and UDim2.fromScale(1,1) or UDim2.fromScale(0,0))
		Checkmark.ImageTransparency = (initialState and 0 or 1)
		ToggleSquare.UIStroke.Color = (initialState and Creator.GetThemeProperty("Accent") or Creator.GetThemeProperty("InElementBorder"))
...
-- VERSÃO FINAL: LÓGICA 100% ORIGINAL E ESTÁVEL + NOVO VISUAL --
...
ElementsTable.Dropdown = (function()
	...
	function Element:New(Idx, Config)
		...
		local DropdownDisplay = New("TextLabel", { -- MUDANÇA: Usando TextLabel para não bloquear cliques
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"), Text = "--", TextColor3 = Color3.fromRGB(240, 240, 240),
			TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Size = UDim2.new(1, -40, 0.5, 0), Position = UDim2.new(0, 8, 0.5, 0),
			AnchorPoint = Vector2.new(0, 0.5), BackgroundTransparency = 1, TextTruncate = Enum.TextTruncate.AtEnd, ThemeTag = { TextColor3 = "Text" },
		})

		local DropdownIco = New("ImageLabel", {
			Image = "rbxassetid://10709790948", Size = UDim2.fromOffset(16, 16), AnchorPoint = Vector2.new(1, 0.5),
			Position = UDim2.new(1, -8, 0.5, 0), BackgroundTransparency = 1, Rotation = 180, ThemeTag = { ImageColor3 = "SubText" },
		})

		local DropdownInner = New("TextButton", {
			Size = UDim2.fromOffset(160, 30), Position = UDim2.new(1, -10, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5),
			BackgroundTransparency = 1, Parent = DropdownFrame.Frame, Text = "", AutoButtonColor = false,
			ThemeTag = { BackgroundColor3 = "Hover" },
		}, {
			New("UICorner", { CornerRadius = UDim2.new(0, 5) }),
			New("UIStroke", { ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Transparency = 0.5, ThemeTag = { Color = "InElementBorder" } }),
			DropdownIco, DropdownDisplay,
		})
		
		local innerMotor, setInnerTransparency = Creator.SpringMotor(1, DropdownInner, "BackgroundTransparency")
		AddSignal(DropdownInner.MouseEnter, function() setInnerTransparency(0.9) end)
		AddSignal(DropdownInner.MouseLeave, function() setInnerTransparency(1) end)

		local SearchBox
		local SearchFrame = New("Frame", {
			Size = UDim2.new(1, 0, 0, 35), BackgroundTransparency = 1, Visible = Dropdown.Searchable, ClipsDescendants = true,
		}, { New("Frame", { Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0,0,1,0), AnchorPoint = Vector2.new(0,1), BackgroundColor3 = Creator.GetThemeProperty("InElementBorder"), BackgroundTransparency = 0.8 }) })
		
		if Dropdown.Searchable then
			SearchBox = Components.Textbox(SearchFrame, false)
			SearchBox.Frame.Size, SearchBox.Frame.Position, SearchBox.Frame.AnchorPoint = UDim2.new(1, -16, 1, -10), UDim2.fromScale(0.5, 0.5), Vector2.new(0.5, 0.5)
			SearchBox.Input.PlaceholderText = "Search..."
		end

		local DropdownListLayout = New("UIListLayout", { Padding = UDim2.new(0, 2), SortOrder = Enum.SortOrder.LayoutOrder })
		local DropdownScrollFrame = New("ScrollingFrame", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, ScrollBarThickness = 4, BorderSizePixel = 0, CanvasSize = UDim2.fromScale(0, 0) },
			{ DropdownListLayout, New("UIPadding", { PaddingTop = UDim2.new(0, 4), PaddingBottom = UDim2.new(0, 4) }) }
		)
		
		local searchHeight = Dropdown.Searchable and 35 or 0
		local DropdownHolderAnim = New("Frame", {
			Size = UDim2.new(1, 0, 1, -searchHeight), Position = UDim2.new(0, 0, 0, searchHeight),
			BackgroundTransparency = 1, ClipsDescendants = true,
		}, { DropdownScrollFrame })
		
		local DropdownHolderCanvas = New("Frame", {
			BackgroundTransparency = 1, Size = UDim2.new(0, 160, 0, 0), Parent = Library.GUI, Visible = false, ZIndex = 50, ClipsDescendants = true,
			ThemeTag = { BackgroundColor3 = "DropdownHolder" },
		}, {
			New("UICorner", { CornerRadius = UDim2.new(0, 6) }),
			New("UIShadow", { ShadowColor = Color3.new(0,0,0), ShadowTransparency = 0.7, ShadowSize = 12, ShadowDistance = 6 }),
			SearchFrame, DropdownHolderAnim,
		})
		table.insert(Library.OpenFrames, DropdownHolderCanvas)

		local function RecalculateListPosition()
			if not DropdownInner or not DropdownInner.Parent then return end
			local dropdownPos, dropdownSize = DropdownInner.AbsolutePosition, DropdownInner.AbsoluteSize
			DropdownHolderCanvas.Position = UDim2.fromOffset(dropdownPos.X, dropdownPos.Y + dropdownSize.Y + 4)
		end
		...
		if Dropdown.Searchable then
			Creator.AddSignal(SearchBox.Input:GetPropertyChangedSignal("Text"), function()
				local Text = SearchBox.Input.Text:lower()
				for _, Element in ipairs(DropdownScrollFrame:GetChildren()) do
					if Element:IsA("TextButton") and Element:FindFirstChild("ButtonLabel") then
		...
		Creator.AddSignal(UserInputService.InputBegan, function(Input)
			if Dropdown.Opened and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then
				...
				if not (mousePos.X >= btnPos.X and mousePos.X <= btnPos.X + btnSize.X and mousePos.Y >= btnPos.Y and mousePos.Y <= btnPos.Y + btnSize.Y) then
					local holderPos, holderSize = DropdownHolderCanvas.AbsolutePosition, DropdownHolderCanvas.AbsoluteSize
					if not (mousePos.X >= holderPos.X and mousePos.X <= holderPos.X + holderSize.X and mousePos.Y >= holderPos.Y and mousePos.Y <= holderPos.Y + holderSize.Y) then
						Dropdown:Close()
					end
				end
			end
		...
		function Dropdown:Open()
			...
			local contentHeight = DropdownScrollFrame.CanvasSize.Y.Offset
			local searchHeight = Dropdown.Searchable and 35 or 0
			local finalHeight = math.min(contentHeight, 220) + searchHeight
			
			local tweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			TweenService:Create(DropdownHolderCanvas, tweenInfo, { Size = UDim2.new(0, DropdownInner.AbsoluteSize.X, 0, finalHeight) }):Play()
			TweenService:Create(DropdownIco, tweenInfo, { Rotation = 0 }):Play()
			if Dropdown.Searchable then SearchBox.Input:CaptureFocus() end
		end
		...
		function Dropdown:Close()
			...
			local tweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
			local anim = TweenService:Create(DropdownHolderCanvas, tweenInfo, { Size = UDim2.new(0, DropdownInner.AbsoluteSize.X, 0, 0) })
			anim.Completed:Connect(function() if not Dropdown.Opened then DropdownHolderCanvas.Visible = false end end)
			anim:Play()
			
			TweenService:Create(DropdownIco, tweenInfo, { Rotation = 180 }):Play()
			if Dropdown.Searchable then SearchBox.Input:ReleaseFocus(false) end
		end

		function Dropdown:Display()
			local Str = ""; if Config.Multi then for Value, _ in pairs(Dropdown.Value) do Str = Str .. Value .. ", " end; if #Str > 0 then Str = Str:sub(1, #Str - 2) end else Str = Dropdown.Value or "" end
			DropdownDisplay.Text = (Str == "" and "--" or Str)
		end
		
		function Dropdown:BuildDropdownList()
			for _, Element in ipairs(DropdownScrollFrame:GetChildren()) do if not (Element:IsA("UIListLayout") or Element:IsA("UIPadding")) then Element:Destroy() end end
			local Buttons = {}
			
			for _, Value in ipairs(Dropdown.Values) do
				local Checkmark = New("ImageLabel", { Image = "rbxassetid://10709790644", Size = UDim2.fromOffset(16, 16), Position = UDim2.new(1, -12, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundTransparency = 1, ImageTransparency = 1, ThemeTag = { ImageColor3 = "Accent" } })
				local ButtonLabel = New("TextLabel", { FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json"), Text = Value, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, BackgroundTransparency = 1, Size = UDim2.new(1, -30, 1, 0), Position = UDim2.fromOffset(12, 0), Name = "ButtonLabel", ThemeTag = { TextColor3 = "Text" } })
				local Button = New("TextButton", { Size = UDim2.new(1, 0, 0, 32), BackgroundTransparency = 1, Text = "", Parent = DropdownListLayout, ThemeTag = { BackgroundColor3 = "Hover" } }, { Checkmark, ButtonLabel, New("UICorner", { CornerRadius = UDim2.new(0, 4) }) })
				...
				local function UpdateButton()
					...
					Checkmark.ImageTransparency = Selected and 0 or 1
				end
				
				AddSignal(Button.MouseEnter, function() setTransparency(0.9) end)
				AddSignal(Button.MouseLeave, function() setTransparency(1) end)
				...
				AddSignal(Button.MouseButton1Click, function()
					...
					UpdateButton(); Dropdown:Display()
					Library:SafeCallback(Dropdown.Callback, Dropdown.Value); if Dropdown.Changed then Library:SafeCallback(Dropdown.Changed, Dropdown.Value) end
					if not Config.Multi then Dropdown:Close() end
				end)
				
				Buttons[Button] = { UpdateButton = UpdateButton }; UpdateButton()
			end
		...
		function Dropdown:SetValue(Val)
			if Dropdown.Multi then
				local nTable = {}; for _, Value in ipairs(Val or {}) do if table.find(Dropdown.Values, Value) then nTable[Value] = true end end
				Dropdown.Value = nTable
			else Dropdown.Value = (Val and table.find(Dropdown.Values, Val)) and Val or nil end
...
ElementsTable.Slider = (function()
	...
	function Element:New(Idx, Config)
		...
		local function updateFromInput(inputObject)
			local position = inputObject.Position
			local railStart = SliderInner.AbsolutePosition.X 
			local railSize = SliderInner.AbsoluteSize.X

			local clampedX = math.clamp(position.X, railStart, railStart + railSize)
			
			local scale = (clampedX - railStart) / railSize
		...
		SliderDot.InputBegan:Connect(onInputBegan)
...
local Icons = {
	...
	["lucide-compass"] = "rbxassetid://88452165400827",
	...
	["lucide-gem"] = "rbxassetid://103722889297145",
	...
	["lucide-home"] = "rbxassetid://117913436030615",
	...
	["lucide-moon"] = "rbxassetid://106832030427465",
	...
	["lucide-paintbrush"] = "rbxassetid://115942226946787",
	...
	["lucide-settings"] = "rbxassetid://98349818138105",
	...
	["lucide-shopping-cart"] = "rbxassetid://122649091403759",
	...
	["lucide-skull"] = "rbxassetid://129171338885512",
	...
	["lucide-syringe"] = "rbxassetid://118889228404010",
	...
	["lucide-user"] = "rbxassetid://77239863455461",
...
function Library:CreateWindow(Config)
	...
	NotificationModule:Init(Window.Root)
...
local MobileMinimizeButton = New("TextButton", {
	...
	New("ImageLabel", {
		Image = Button_Icon ~= "" and Button_Icon or "rbxassetid://10734897102",
		Size = UDim2.new(0.8, 0, 0.8, 0),
		Position = UDim2.new(0.5, 0, 0.5, 0),
		AnchorPoint = Vector2.new(0.5, 0.5),
		BackgroundTransparency = 1,
...
Creator.AddSignal(RunService.Heartbeat, function()
	if isDragging and dragStart and dragOffset and Minimizer and Minimizer.Parent then
		...
		Minimizer.Position = UDim2.new(0, newX, 0, newY)
	end
end)
